...
function winLoseRatio(playerCards,player2Cards){
    ...
    for (var i = 0;i<player2Cards.length;i++){
        ...
        try{
            ...
            if(player.getName() == "Player #1"){
                wins.push(Number(player.getWinsPercentage()))
                ties.push(Number(player.getTiesPercentage()))
            }
    ...
    console.log(wins)
    console.log(ties)
    const averageWins = average(wins).toFixed(2)
    const averageTies= average(ties).toFixed(2)
    
    return [averageWins,averageTies]

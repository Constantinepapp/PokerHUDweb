...
const Equity = () =>{
    ...
    const { win, loose, tie } = raceRange(combo, ranges, 1E4)
    ...
    const setComboChanger = () =>{
        ...
        const selection = bestCombo.slice(0,slider)
        var newState = []
    ...
    return(
        ...
        <div  className="slider">
            <label>{slider}</label>
            <Slider style={{"marginTop":'30px',"marginBottom":'20px'}} min={0} max={bestCombo.length-1} value={slider}  step={1} onChange={setSlider}/>
        </div>
